...
func (h *GuestHandler) List(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		l.Error("error fetching guests from db", zap.Error(err))
		InternalServerError(w, nil)
		return
	}
	l.Info("guests rendered", zap.Int("amount", len(es)))
	easyjson.MarshalToHTTPResponseWriter(NewGuest3946703519Views(es), w)
}
